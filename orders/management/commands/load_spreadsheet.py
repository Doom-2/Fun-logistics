...
def gsheet2df(spreadsheet_name, sheet_num):
    """ Load Google spreadsheet to Pandas DataFrame"""
    ...
    # Retrieves the credentials from .json and authorize access to Google spreadsheets
    credentials = sac.from_json_keyfile_name(f'{CUR_DIR}/google_api_credentials.json', scope)
    client = gspread.authorize(credentials)

    # Opens a specific spreadsheet, pulls all the records as a dictionary
    # and converts it into a DataFrame, renaming the columns to the same ones as in 'Order' model.
